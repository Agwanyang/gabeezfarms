...
function Fertilizers() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        await updateDoc(doc(db, 'fertilizers', editId), data)
        setEditId(null)
      } else {
        await addDoc(collection(db, 'fertilizers'), { ...data, createdAt: serverTimestamp() })
      }
      setForm({ fertilizerName: '', quantity: '', unit: 'KG', costPerUnit: '', greenhouse: '', cropName: '', dateApplied: '' })
      setSuccess(true)
      setShowForm(false)
  ...
  return (
    <PageLayout>
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Fertilizer Usage</h2>
          <p className="text-slate-500 mt-1 text-sm">Track all fertilizer applications and costs</p>
        </div>
        <button
          onClick={() => { setShowForm(!showForm); setEditId(null); setForm({ fertilizerName: '', quantity: '', unit: 'KG', costPerUnit: '', greenhouse: '', cropName: '', dateApplied: '' }) }}
          ...
          {showForm ? 'Close Form' : '+ Add Fertilizer Record'}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-xl p-5 shadow-sm border-l-4 border-amber-400">
          <p className="text-sm text-slate-500 mb-1">Total Fertilizer Cost</p>
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border-l-4 border-purple-500">
          <p className="text-sm text-slate-500 mb-1">Fertilizers Used</p>
          <p className="text-2xl font-bold text-slate-800">{new Set(records.map(r => r.fertilizerName)).size}</p>
        </div>
      </div>

      {success && (
        <div className="bg-green-50 border border-green-200 text-green-700 p-4 rounded-xl mb-6 font-medium">
          Fertilizer record saved successfully!
        </div>
      )}
      ...
      {showForm && (
        <div className="bg-white rounded-xl p-6 mb-8 shadow-sm border border-slate-200">
          <h3 className="text-base font-semibold text-slate-800 mb-6 pb-4 border-b">
            {editId ? 'Edit Fertilizer Record' : 'New Fertilizer Application'}
          </h3>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
              <label className="block text-sm font-medium text-slate-600 mb-2">Fertilizer Name</label>
              <input name="fertilizerName" value={form.fertilizerName} onChange={handleChange}
                ...
                placeholder="e.g. NPK 15-15-15" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-600 mb-2">Crop Applied To</label>
            ...
            <div className="sm:col-span-2 flex gap-3">
              ...
              {editId && (
                <button type="button" onClick={() => { setEditId(null); setShowForm(false); setForm({ fertilizerName: '', quantity: '', unit: 'KG', costPerUnit: '', greenhouse: '', cropName: '', dateApplied: '' }) }}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200">
        <div className="p-5 border-b border-slate-100">
          <h3 className="text-base font-semibold text-slate-800">All Fertilizer Records</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[720px]">
            <thead>
              <tr className="bg-slate-50 text-left">
                <th className="px-5 py-4 text-slate-500 font-medium">Fertilizer</th>
                <th className="px-5 py-4 text-slate-500 font-medium">Crop</th>
              ...
              {records.length === 0 ? (
                <tr>
                  <td colSpan="8" className="px-6 py-16 text-center text-slate-400">
                    ...
                    <div className="font-medium">No fertilizer records yet</div>
                    <div className="text-xs mt-1">Click "Add Fertilizer Record" to get started</div>
                  </td>
                </tr>
              ) : (
                records.map(record => (
                  <tr key={record.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                    <td className="px-5 py-4 font-semibold text-slate-800">{record.fertilizerName}</td>
                    <td className="px-5 py-4 text-slate-600">{record.cropName}</td>
                    ...
                    <td className="px-5 py-4 font-semibold text-amber-600">{'₦'}{record.totalCost?.toLocaleString()}</td>
