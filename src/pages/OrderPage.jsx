...
function OrderPage() {
  ...
  if (orderPlaced) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#052e16] via-[#064e3b] to-[#065f46] flex items-center justify-center p-5 font-sans">
        <style>{`
          @keyframes popIn { 0% { transform: scale(0.8); opacity: 0; } 100% { transform: scale(1); opacity: 1; } }
          @keyframes checkBounce { 0%, 100% { transform: scale(1); } 50% { transform: scale(1.15); } }
          .popin { animation: popIn 0.5s cubic-bezier(0.34,1.56,0.64,1); }
          .checkbounce { animation: checkBounce 0.6s ease 0.4s; }
        `}</style>
        <div className="popin bg-white rounded-3xl p-8 sm:p-10 text-center max-w-md w-full shadow-2xl">
          <div className="checkbounce text-6xl mb-4">✅</div>
          <h2 className="text-2xl font-extrabold text-slate-800 mb-2">Order Placed!</h2>
          <p className="text-slate-500 text-sm mb-6">Thank you for your order. We'll contact you shortly to confirm delivery.</p>
          <div className="bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-2xl p-4 mb-7">
            <p className="text-xs text-slate-500 uppercase tracking-wide mb-1">Order Reference</p>
            <p className="text-2xl font-extrabold text-green-700">#{orderId}</p>
          </div>
          <button onClick={() => setOrderPlaced(false)}
            className="bg-gradient-to-br from-green-500 to-green-600 text-white rounded-full px-9 py-3.5 font-bold text-sm shadow-lg shadow-green-500/30 transition-transform hover:scale-105">
            Shop Again
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      {toast && (
        <div className="toast-in fixed top-5 left-1/2 -translate-x-1/2 z-[200] bg-slate-800 text-white px-5 py-3 rounded-full text-sm font-semibold shadow-2xl flex items-center gap-2">
          <span className="text-green-400 text-base">✓</span> {toast}
        </div>
      )}

      <div className="relative overflow-hidden bg-gradient-to-br from-[#052e16] via-[#0f4c2a] to-[#1a7a42] px-4 py-5 sm:px-8 sm:py-7 text-white">
        <div className="absolute -top-16 -right-16 w-56 h-56 rounded-full bg-green-400/15 blur-3xl" />
        <div className="max-w-[1100px] mx-auto flex justify-between items-center relative">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-2xl">🌿</span>
              <h1 className="text-lg sm:text-xl font-extrabold tracking-tight">Gabeez Green Farms</h1>
            </div>
            <p className="text-white/65 text-xs sm:text-sm">Fresh from our greenhouse to your table</p>
          </div>
          <div className="relative cursor-pointer" onClick={() => cart.length > 0 && setStep('checkout')}>
            <div className="bg-white/10 backdrop-blur-md rounded-2xl px-3 py-2 sm:px-5 sm:py-2.5 flex items-center gap-2 border border-white/15">
              <span className="text-lg sm:text-xl">🛒</span>
              <span className="font-bold text-xs sm:text-sm">{cart.length} items</span>
            </div>
            {cart.length > 0 && (
              <div className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-[11px] font-bold shadow-md shadow-red-500/50">
                {cart.length}
              </div>
            )}
          </div>
      ...
      {step === 'shop' && (
        <div className="relative h-28 sm:h-36 overflow-hidden">
          <img src={greenhouse} alt="Our Greenhouse" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-r from-[#052e16]/75 to-[#052e16]/20 flex items-center px-4 sm:px-8">
            <div className="max-w-[1100px] mx-auto w-full">
              <p className="text-green-300 text-[11px] sm:text-xs font-bold tracking-widest uppercase mb-1">Straight From The Greenhouse</p>
              <p className="text-white text-sm sm:text-lg font-bold">Harvested fresh, delivered the same day</p>
            </div>
          </div>
        </div>
      )}

      <div className="max-w-[1100px] mx-auto px-4 sm:px-5 pt-6 sm:pt-8 pb-24">

        {step === 'shop' && (
          <div className="gf-step-fade">
            <div className="flex items-center gap-3 mb-1">
              <div className="w-1 h-6 bg-gradient-to-b from-green-600 to-green-700 rounded-sm" />
              <h2 className="text-xl sm:text-2xl font-extrabold text-slate-800">Our Products</h2>
            </div>
            <p className="text-slate-500 text-sm ml-4 mb-6 sm:mb-7">Fresh greenhouse produce, harvested daily</p>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {products.map(product => (
                <div key={product.id} className={'gf-product-card bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100' + (product.available ? '' : ' opacity-60')}>
                  <div className="relative h-40 sm:h-44 overflow-hidden">
                    <img
                      src={product.image}
                      alt={product.name}
                      className={'gf-product-img w-full h-full object-cover' + (product.available ? '' : ' opacity-50')}
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/35 to-transparent" />
                    <span className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm text-green-700 text-[11px] font-bold px-3 py-1 rounded-full">
                      {product.tag}
                    </span>
                  </div>
                  <div className="p-4 sm:p-5">
                    <h3 className="text-sm font-bold text-slate-800 mb-1.5">{product.name}</h3>
                    <p className="text-lg font-extrabold text-green-600 mb-4">
                      ₦{product.price.toLocaleString()} <span className="text-xs text-slate-400 font-medium">per {product.unit}</span>
                    </p>
                    {product.available ? (
                      <button onClick={() => addToCart(product)}
                        className="w-full bg-gradient-to-br from-green-500 to-green-600 text-white rounded-xl py-2.5 font-bold text-xs transition-all hover:scale-[1.03] hover:shadow-lg hover:shadow-green-500/35">
                        + Add to Cart
                      </button>
                    ) : (
                      <div className="w-full bg-slate-100 text-slate-400 rounded-xl py-2.5 font-bold text-xs text-center">
                        Out of Stock
                      </div>
                    )}
                  </div>
            ...
            {cart.length > 0 && (
              <div className={
                'fixed bottom-5 left-1/2 -translate-x-1/2 bg-gradient-to-br from-slate-800 to-slate-900 text-white rounded-full pl-5 pr-2 py-2 sm:pl-6 sm:pr-2.5 sm:py-2.5 flex items-center gap-3 sm:gap-5 shadow-2xl z-[100] transition-all duration-300 w-[92%] sm:w-auto justify-between ' +
                (cartBarVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8')
              }>
                <span className="text-xs sm:text-sm whitespace-nowrap">🛒 {cart.length} item{cart.length > 1 ? 's' : ''} — <strong>₦{totalAmount.toLocaleString()}</strong></span>
                <button onClick={() => setStep('checkout')}
                  className="bg-gradient-to-br from-green-500 to-green-600 text-white rounded-full px-4 sm:px-6 py-2 font-bold text-xs whitespace-nowrap">
                  Checkout →
                </button>
              </div>
            )}
          ...
          <div className="gf-step-fade">
            <div className="flex items-center gap-2 mb-6 overflow-x-auto">
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <span className="text-xs text-green-600 font-bold">Shop</span>
              </div>
              <div className="w-8 h-0.5 bg-green-200 shrink-0" />
              <div className="flex items-center gap-1.5 shrink-0">
                <div className="w-6 h-6 rounded-full bg-green-600 text-white flex items-center justify-center text-xs font-bold">2</div>
                <span className="text-xs text-green-600 font-bold">Details</span>
              </div>
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <span className="text-xs text-slate-400 font-semibold">Confirmed</span>
              </div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6 lg:gap-7">
              <div>
                <h2 className="text-lg sm:text-xl font-extrabold text-slate-800 mb-5">Your Details</h2>
                  ...
                  <form onSubmit={handleOrder}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <input name="customerName" value={form.customerName} onChange={handleChange}
                          className="w-full border-[1.5px] border-slate-200 rounded-xl px-3.5 py-2.5 text-sm outline-none focus:border-green-600 transition-colors"
                          placeholder="John Doe" required />
                      </div>
                      <div>
                    ...
                    <div className="mb-4">
                      ...
                      <textarea name="address" value={form.address} onChange={handleChange}
                        className="w-full border-[1.5px] border-slate-200 rounded-xl px-3.5 py-2.5 text-sm outline-none focus:border-green-600 transition-colors resize-none"
                        placeholder="Enter your delivery address" rows="3" required />
                    </div>
                    <div className="mb-6">
                    ...
                    <div className="flex gap-3">
                      <button type="button" onClick={() => setStep('shop')}
                        className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl py-3 font-bold text-sm transition-colors">
                        ← Back
                      </button>
                      <button type="submit" disabled={loading}
                        className="flex-[2] bg-gradient-to-br from-green-500 to-green-600 text-white rounded-xl py-3 font-bold text-sm shadow-lg shadow-green-500/30 transition-transform hover:scale-[1.02] disabled:opacity-70">
                        {loading ? 'Placing Order...' : 'Place Order'}
                      </button>
                    </div>
                  </form>
              ...
              <div>
                <h2 className="text-lg sm:text-xl font-extrabold text-slate-800 mb-5">Order Summary</h2>
                <div className="bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-slate-100">
                  {cart.map(item => (
                    <div key={item.id} className="flex justify-between items-center py-3.5 border-b border-slate-100 last:border-b-0">
                      <div>
                        <div className="flex items-center gap-2.5 mb-1.5">
                          <img src={item.image} alt={item.name} className="w-8 h-8 rounded-lg object-cover shadow" />
                          <p className="text-sm font-semibold text-slate-800">{item.name}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <button onClick={() => updateQty(item.id, item.quantity - 1)}
                            className="w-6 h-6 border border-slate-200 rounded-full bg-white text-sm">-</button>
                          <input
                            type="number"
                            min="1"
                            value={item.quantity}
                            ...
                            className="w-12 text-center text-xs text-slate-800 border border-slate-200 rounded-md py-1 outline-none"
                          />
                      ...
                      <div className="text-right">
                        ...
                        <button onClick={() => removeFromCart(item.id)}
                          className="text-[11px] text-red-500 font-semibold">Remove</button>
                      </div>
                    </div>
                  ))}
                  <div className="flex justify-between items-center pt-4">
                    <span className="text-base font-extrabold text-slate-800">Total</span>
                    <span className="text-xl font-extrabold text-green-600">₦{totalAmount.toLocaleString()}</span>
                  </div>
                </div>
              </div>
            </div>
